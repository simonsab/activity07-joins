...
**Response**:
There are La Quinta's outside the U.S. (example New Zealand) and there are Denny's outside U.S.
...
**Response**:
Drop observations outside the U.S. Possibly use a semi-join to filter out data we don't want?
...
```{r dennys_anti_join, message = FALSE}
dennys %>%
...
```{r laquinta_anti_join, message = FALSE}
laquinta %>%
  anti_join(states, by=c("state"="abbreviation"))
```
